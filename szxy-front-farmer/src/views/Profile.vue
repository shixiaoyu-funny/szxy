  ...
  <div class="profile-container">
    ...
    <div class="profile-content">
      ...
      <div class="function-list">
        ...
        <div class="function-item" @click="goToAccess">
          <span class="function-icon">📝</span>
          <span class="function-text">景点上报</span>
          <span class="function-arrow">→</span>
        </div>
        <div v-if="isChief" class="function-item" @click="goToVillageFarmers">
          <span class="function-icon">👥</span>
          <span class="function-text">本村农户管理</span>
          <span class="function-arrow">→</span>
        </div>
        <div v-if="villageId" class="function-item" @click="goToDetail">
          <span class="function-icon">🏡</span>
          <span class="function-text">所属农村详情</span>
          <span class="function-arrow">→</span>
        </div>
...
import { getUserInfo, getVillageInfo } from '../api/user';
...
const goToDetail = () => {
  if (villageId.value) {
    router.push(`/village/${villageId.value}`);
  }
};

const goToVillageFarmers = () => {
  router.push('/village-farmers');
};
...
// 农户端仅允许农户（type === 2）；与后端不一致时退回登录并提示
const FARMER_APP_EXPECTED_TYPE = 2;
...
const loadUserInfo = async () => {
  try {
    ...
    if (res.data) {
      if (res.data.type !== FARMER_APP_EXPECTED_TYPE) {
    ...
    const d = v.data as { id?: number } | null | undefined;
