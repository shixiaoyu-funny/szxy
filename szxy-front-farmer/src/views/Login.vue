<template>
  <div class="login-container">
    <div class="login-card">
      ...
      <p class="login-subtitle">AI 驱动乡村振兴服务平台 - 农户端</p>
...
const phoneLogin = async () => {
  ...
  try {
    ...
    // 校验是否为农户（type === 2）
    if (userInfo.type !== 2) {
      errorMessage.value = '权限校验失败，仅农户可登录';
      userStore.logout();
      loading.value = false;
      return;
    }
    ...
    // 检查农户是否已有归属农村
    try {
      const villageRes = await getVillageInfo();
      const hasVillage = !!villageRes.data;

      if (hasVillage) {
        // 如果有归属农村，直接跳转到首页
        router.push('/');
      } else {
        // 如果没有归属农村，跳转到农村管理页面进行申请
        router.push('/village-manage');
      }
    } catch (villageError) {
      console.error('检查农村归属信息失败:', villageError);
      // 如果检查失败，默认跳转到农村管理页面
      router.push('/village-manage');
    }
  } catch (error) {
