<template>
  <div class="my-likes-container">
    ...
    <div v-else-if="error" class="error-container">
      <p>{{ error }}</p>
      <button class="btn btn-primary" @click="fetchLikes">重试</button>
    </div>
...
const fetchLikes = async () => {
  ...
  try {
    ...
    if (res.data) {
      ...
      ElMessage.success('获取历史点赞景点列表成功');
    }
  } catch (err) {
    console.error('获取历史点赞景点列表失败:', err);
    error.value = '获取历史点赞景点列表失败，请重试';
    ElMessage.error('获取历史点赞景点列表失败，请重试');
  } finally {
    loading.value = false;
  }
};
